...
#### Параграф 1: Многопоточные (concurrent) структуры данных и алгоритмы.
...
#### Параграф 2: Параллельные (parallel) алгоритмы.
...
Считаем, что нам дан цикл _pfor_ (parallel for), который сам занимается вопросами _scheduling_
...
#### Параграф 2: Модель Fork-Join.

Вся наша программа &mdash; набор разветвлений/слияний, между которыми исполнение последовательно.

Поэтому любую модель вычислений можно представить в виде _DAG_ (Directed Acyclic Graph).
